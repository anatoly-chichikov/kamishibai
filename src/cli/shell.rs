...
impl Shell<LiveCardGenerator> {
    ...
    pub(super) fn startup(
        ...
        if let Some(session) = shell.session.as_mut()
            && !session.claim(&shell.app)?
        {
            bail!("the session is being generated by another process; cancel it first");
        }
    ...
    P: CardWorkflow,
{
    ...
    /// Take the session's liveness lock before any generation starts; on refusal
    /// show why and report false so the caller never starts the engine.
    fn claim_session(&mut self) -> bool {
        let Some(session) = self.session.as_mut() else {
            return true;
        };
        match session.claim(&self.app) {
            Ok(true) => true,
            Ok(false) => {
