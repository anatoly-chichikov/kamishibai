...
3. **Record the synthetic state shots** (state-walker, no Gemini calls):
   ...
   `live/22-s12-label-legacy-meta.png`, plus the open batch-settings pair
   `live/23-batch-sentence-settings.png` and
   `live/24-batch-sentence-settings-narrow.png`. All are 2x except S10 and the narrow
...
The review, six environment/modal/failure/retry PNGs, two batch-settings PNGs,
and twelve sentence-label scenario PNGs listed in step 3 are produced reproducibly by `states.tape`
...
S10–S12 are indices 18–20, the retry stress gallery is index 21, and the open
batch sentence-settings editor is index 22. When the design changes, edit the demo data in
