...
Initial batch generation guidance is separate from that post-generation rewrite transaction. `SentenceBatchSettings` persists beside the reviewed candidates in `session.json` as per-session provenance; its default is no level plus `best-fit` example formats, which leaves the metadata prompt and provider call count unchanged. Every actual TUI choice on `WhatIUnderstood` is also atomically remembered in `preferences.json` under the canonical learning-language code. Fresh understanding loads that target's override, returning both axes to `best-fit` removes it, and a resumed session keeps its own recorded settings. An explicit level becomes one initial pinned axis on every card; `statements`, `questions`, and `dialogue` pin one exact format throughout the batch, while `mixed` deterministically allocates one statement, one question, and one dialogue per complete group of three. The pending per-card metadata request persists only until metadata succeeds; the permanent batch setting remains in both session and result JSON as provenance. TUI and console (`new --level … --types questions`) must allocate through the same policy; old `natural` and `varied` values remain read/CLI aliases only.
...
The viewport rides the engine: a new or restarted batch arms following (`cards_started` and `restart_regeneration` both do), `cards_running` then carries the selection onto the card being built, and `Shell::follow_running_card` scrolls to it once per loop pass beside the existing clamp. The ride holds that card `FOLLOW_ANCHOR_PERCENT` (30%) of the way down the viewport rather than against its bottom row, because what sits under the running card is the grey tail of words the batch has not reached yet — the only place a live build says what is coming next — and parking the card on the last line left nothing of it on screen. `App::follow_anchor` answers with that offset and, like the editor anchor beside it, applies unconditionally, so the ride cannot be intercepted by the plain keep-it-visible branches. It yields to the old bottom park only when the card's own block is taller than the room the anchor leaves under it, which is the one case where holding it high would push its later rows off a short terminal. The ride still aims at the **finished** card, not at the stub drawn so far: a followed card reserves its head plus all three artifact rows plus its trailing blank (`focused_card_range`), and the clamp in `body_scroll_to_selection` widens to that range when it reaches past the rows drawn today, so the scroll does not creep under the card while it fills in. At the tail of a batch there is no longer 70% of a viewport of content below the card, and the existing content clamp lets it settle lower — the anchor is what the ride aims for, not a guarantee. Any manual scroll or arrow breaks the ride, and the ride also *ends* where the batch ends: `body_scroll_clamped` parks the view at the end of the content once `App::ride_settled` holds — the run finished on its own, nothing is expanded, and the outcome strip is up — and disarms following in the same breath. Without that, the strip appears in the very moment the batch settles and takes its rows out of the card area from above, sliding the card the run just finished under the block announcing it; parking once rather than every frame is what keeps the correction from fighting a later click or an opened card. Beyond those, the per-frame clamp and the snap itself must never break the ride. The ride is also what makes the pace legible, and the shell holds each artifact step on screen for at least `STEP_DWELL` (75ms) before `Shell::poll_artifact` takes its result: a cached step answers in a millisecond and `advance_engine` would raise the next one inside the same tick, so a whole card walked meta → sound → scene → picture between two frames and the batch flashed past unreadably. The dwell is three `FAST_JOB_POLL`s and is written that way: while a step is in flight the slot stays incomplete, so `poll_timeout` holds the loop at the fast cadence and the floor can only ever expire on that 25ms grid — a value off the grid would quietly round up to it. Being shorter than an animation frame, the dwell paces the rows rather than the spinner: a step row is painted the moment `spawn_artifact` raises it, and the floor is what keeps it on screen. The floor is unconditional, because a real provider call outlasts it many times over and never feels it, and it lives on `Shell` alone, so `console::produce` and the background worker keep running at full speed. An expanded **focused** card suppresses following rather than clearing it, so collapsing (or walking off) that card resumes the ride; a ride that lands on a parked expanded card pauses the same way until that card is collapsed. `↑↓` are the only keys that move the card cursor: a jump to the next unfinished card existed on `Tab`/`Shift+Tab` and was removed, because during a build the viewport already rides the engine and afterwards a broken card announces itself with the one bright `✗` in its block while `Ctrl+G` re-rolls every failure at once — nobody has to walk to them. `Tab` and `BackTab` therefore map to no event at all (`to_app` in `src/tui/input.rs`). Card numbers are absolute batch positions and never renumber.
