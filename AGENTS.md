...
cache hit. The active row shows the spinner plus `ai is working…`; a terminal
failure is a bare `✗` plus any known cost, with no `gave up` to push the money
out of the shared column; a discarded artifact `⊘ discarded`; an inactive
...
kind, and level appear as three consecutive tags at one fixed column two cells
past the value column, on the `voice` row; at narrow widths whole tags may
continue at that same column on the `manga` row, and when the complete atomic
sequence cannot fit — including while the `voice` row itself is busy and its
`ai is working…` reaches into the tag column — the whole summary hides while
...
below the three rows, separated from `manga` by exactly one
